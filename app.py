...
def main():
    ...
    alliance_filter = st.sidebar.multiselect(
        "Selecciona alianza(s):", options=alliance_options, default=alliance_options
    )
    ...
    min_matches = st.sidebar.slider(
        "Mínimo de matches jugados:",
        min_value=0,
        max_value=int(df_pandas["matches_played"].max()),
        value=0,
    )
    ...
    metric_label = st.selectbox(
        "Selecciona la métrica a visualizar:", options=list(metric_options.keys())
    )
